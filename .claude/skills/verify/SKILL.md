...
- Dev server talks to the **live prod Supabase** via `.env.local` — any RSVP you submit writes real rows. Clean up test RSVPs via the in-page withdraw flow ("Can't make it anymore?" → "Yes, free up my spot"); note the withdraw posts a visible "X is no longer attending" system message into Activity Chatter that cannot be removed from the UI.
