...
|------|---------|---------|
| 2026-09-09 | **Baseball batting-cage hero on `/a/[code]`** | `activity-images.ts` gains a `baseball\|softball\|batting` → `baseball` keyword rule (above the racquet rules) so "SWING FOR IT — BASEBALL CAGE!" @ Swing! Batting Centre (share `28674DB3`, Entertainment category) no longer falls through to the generic `social` vibe photo. New `public/images/activities/baseball.jpg` (1200×1500, ~70 KB) is an **illustrated interim** rendered locally from SVG via sharp (backlit batter mid-swing, cage netting, amber lamps, orange rim glow) — every stock/photo source was unreachable from the sandbox (Unsplash/Pexels/Wikimedia blocked by egress policy, Gamma out of credits, nothing in Drive/Slack/Gmail/Notion/Supabase storage). Swap in a licensed batting-cage photo at the same path; no code change. Resolver checked on the live RPC fields; padel/cafe/fallback unchanged; `tsc` 0, ESLint 0. |
