...
|------|---------|---------|
| 2026-06-28 | **Design refinement Batch C (consistency + shared-component polish)** | Continues the audit-first refinement pass (`docs/DESIGN_REFINEMENT_AUDIT.md`); still no layout/structure changes, emojis preserved. (1) **FindYourVibe** download CTA: raw styled `<a>` → `Button` component (consistent hover/active/focus + brand shadow). (2) **Hero**: collapsed the two near-identical TestFlight/waitlist primary-CTA branches into one button driven by a computed `primaryCta` ({href,label,id,onClick}), −~20 duplicated lines. (3) **Navigation (G6)**: removed the `if (!mounted) return null` gate so the nav is present in **SSR markup** (was absent on first paint / for crawlers); pre-scroll bg is transparent regardless of theme so there's no visible hydration delta, `suppressHydrationWarning` guards the scrolled-refresh edge. Verified: `<nav>` in raw server HTML + 0 hydration errors. (4) **JoinMovement**: glass quote card got a 1px inner-edge refraction highlight (`shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]`); removed a pre-existing dead `defaultTransition` import. (5) **G8 + completes G2**: shared `Heading` (`Typography.tsx`) + `SectionHeader` (`SectionWrapper.tsx`) — the last two spots with the inline `style={{fontFamily:'Satoshi'}}` pattern — now use the `font-heading` token + `tracking-tight`. Since these render headings across about/faq/gamification/safety/etc., this applies the font-fallback restore + tighter display tracking **site-wide**, not just the homepage. Browser-verified via Playwright (light/dark/scroll); `tsc --noEmit` + ESLint clean; 0 console errors. Branch `claude/web-visual-polish`. Homepage refinement backlog now essentially complete; remaining = inner pages. |
