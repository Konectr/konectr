...
**Last Updated**: 2026-07-14 | **Status**: Production Live (V1.0) | **FAQ**: v3 (58 questions) | **RSVP `/a/[code]`**: Kinetic redesign LIVE (2026-07-07) | **Web Activity Chatter**: text-only feed (system/location messages hidden, 2026-07-14)
...
|------|---------|---------|
| 2026-07-14 | **Web Activity Chatter → text-only feed (system/location messages no longer leak to the public share page)** | **Bug:** on `/a/[code]` the "Activity Chatter" panel showed `message_type='system'` group-chat notices — e.g. "*Claude Test is no longer attending*" — rendered as a normal guest bubble (orange name + "via web" badge). **Root cause:** `get_web_chat_messages` RPC returned **every** message in the group conversation with no type filter, and the withdrawal notice inserted by `cancel_web_rsvp` carries the cancelled RSVP's `web_rsvp_id`, so the RPC set `is_from_web=true` + `sender_display_name`=the guest's name. **Fix (whitelist `message_type='text'`):** (1) **DB** — `get_web_chat_messages` now filters `AND m.message_type = 'text'` (excludes `system` notices **and** `location_live`/`location_static` pins, which rendered as empty bubbles); cap counter also narrowed to the guest's own `text` sends. Applied to production (`Konectr-App` / `rsrplvdtbycqcxjlyeju`) via MCP; mirrored to `supabase/migrations/20260714000000_web_chat_exclude_non_text_messages.sql` (**back-port to the mobile repo's migration history**). In-app mobile group chat is **unaffected** (it reads the `messages` table directly, not this RPC, so members still see join/leave notices). (2) **Web defense-in-depth** — `/api/chat/history` re-filters out any non-`text` message before returning, and `WebChatPanel`'s `ChatMessage` type gained `message_type?`, so a future RPC regression can't leak system rows again. `tsc`/ESLint clean on changed files. Also removed the one live "Claude Test is no longer attending" row (id `30d33a64…`) from the Mantra Rooftop (`F110F31F`) chat; the two other leftover "no longer attending" test notices on another activity are now hidden by the same filter (no deletion needed). |
