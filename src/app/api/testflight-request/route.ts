...
// API route: POST /api/testflight-request
// Captures iOS TestFlight beta-invite requests from the web share-link RSVP page.
// Anon Supabase client → register_testflight_request RPC (SECURITY DEFINER).
// Mirrors /api/android-waitlist; distinct referral_source for attribution.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc('register_testflight_request', {
      p_email: normalized,
